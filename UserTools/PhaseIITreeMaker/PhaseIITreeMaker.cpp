...
void PhaseIITreeMaker::LoadAllTankHits(bool IsData) {
  ...
  m_data->Stores.at("RecoEvent")->Get("RecoDigit", fDigitList);
  if (!got_hits){
  ...
  for(RecoDigit &adigit : *fDigitList){
	  fHitX.push_back(adigit.GetPosition().X());
	  fHitY.push_back(adigit.GetPosition().Y());
	  fHitZ.push_back(adigit.GetPosition().Z());
	  fHitT.push_back(adigit.GetCalTime());
	  fHitQ.push_back(adigit.GetCalCharge());
  }
